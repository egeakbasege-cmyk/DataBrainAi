...
interface Props { analysis: AnalysisRecord }

export function MetadataPanel({ analysis }: Props) {
  const { platform, rawData, aiAnalysis } = analysis
  const raw = rawData as any
  const ai  = aiAnalysis as any

  if (platform === 'SHOPIFY') return <ShopifyMeta raw={raw} ai={ai} />
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {topTags.map(([tag, count]) => (
              <span
                key={tag}
                className="text-xs px-2 py-0.5 rounded-md bg-zinc-900 border border-zinc-800 text-zinc-400"
              >
      ...
      <p className="text-xs text-zinc-700 flex items-center gap-1">
        <Calendar size={10} />
        Scraped {new Date(raw.scrapedAt).toLocaleString()}
      </p>
    </div>
  )
}
...
function AmazonMeta({ raw, ai }: { raw: any; ai: any }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-0.5">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star
              key={i}
              size={14}
              className={i < fullStars ? 'text-amber-400 fill-amber-400' : i === fullStars && hasHalf ? 'text-amber-400 fill-amber-400/50' : 'text-zinc-700'}
            />
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: 'Buy Box Price', value: raw.price ? formatCurrency(raw.price, raw.currency) : 'N/A', icon: DollarSign },
          { label: 'Threat Score',  value: `${ai.competitorScore ?? '—'} / 100`, icon: Star },
