...
import { cn, SEVERITY_COLOR, EFFORT_COLOR } from '@/lib/utils'
...
export function AIPlaybook({ analysis }: Props) {
    ...
    <div className="bg-[#0f0f12] border border-zinc-800 rounded-xl overflow-hidden h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {activeTab === 'vulnerabilities' && (
          <VulnerabilitiesTab vulnerabilities={analysis.vulnerabilities} />
        )}
        {activeTab === 'battleplan' && (
          <BattlePlanTab steps={analysis.actionableBattlePlan} />
        )}
        {activeTab === 'adscript' && (
          <AdScriptTab script={analysis.adCreativeScript} />
        )}
        {activeTab === 'seo' && (
          <SeoTab keywords={analysis.seoKeywordsToTarget} />
        )}
      </div>
    </div>
  )
...
function VulnerabilitiesTab({ vulnerabilities }: { vulnerabilities: AIAnalysis['vulnerabilities'] }) {
  return (
    <>
...
function BattlePlanTab({ steps }: { steps: AIAnalysis['actionableBattlePlan'] }) {
  return (
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-3 flex-1 mb-1">
            <div className="flex items-center justify-between gap-2 mb-1">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className={cn('text-xs font-medium', EFFORT_COLOR[s.effort])}>
                  {s.effort} effort
                </span>
...
function AdScriptTab({ script }: { script: AIAnalysis['adCreativeScript'] }) {
  return (
    <div className="space-y-3">
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2">
        {keywords.map((kw, i) => (
          <span
            key={i}
            className="text-xs px-3 py-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 font-medium"
          >
            {kw}
          </span>
        ))}
