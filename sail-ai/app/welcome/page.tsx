      ...
      <div style={{ padding: '1rem' }}>
        ...
        <div style={{ marginBottom: '0.875rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.6rem', color: 'rgba(255,255,255,0.35)' }}>Mevcut CVR</span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '3px' }}>
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.6rem', color: 'rgba(255,255,255,0.25)' }}>Sektör medyanı</span>
...
function SignupSection({ onSignedUp }: { onSignedUp: () => void }) {
  ...
  async function handleEmailRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.message ?? 'Kayıt başarısız.'); setLoading(false); return }
      setSuccess(true)
  ...
  if (success) {
    return (
      <div style={{ textAlign: 'center', padding: '2rem' }}>
        ...
        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', color: '#71717A' }}>Hesabınız oluşturuldu, yönlendiriliyorsunuz…</p>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.7rem', color: '#A1A1AA' }}>ya da e-posta ile</span>
        <div style={{ flex: 1, height: 1, background: 'rgba(12,12,14,0.09)' }} />
      </div>
      ...
      {!emailMode ? (
        ...
        <form onSubmit={handleEmailRegister} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <input type="text" placeholder="Adınız" value={name} onChange={e => setName(e.target.value)} required style={inp} />
          <input type="email" placeholder="E-posta" value={email} onChange={e => setEmail(e.target.value)} required style={inp} />
          <input type="password" placeholder="Şifre (min 8 karakter)" value={password} onChange={e => setPassword(e.target.value)} required style={inp} />
      ...
      <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.65rem', color: '#A1A1AA', textAlign: 'center', margin: 0, lineHeight: 1.6 }}>
        Hesap oluşturarak{' '}
        <span style={{ textDecoration: 'underline', cursor: 'pointer' }}>Kullanım Koşulları</span>
        {' '}ve{' '}
        <span style={{ textDecoration: 'underline', cursor: 'pointer' }}>Gizlilik Politikası</span>
        {' '}kabul edilmiş sayılır.
      </p>

      <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.7rem', color: '#71717A', textAlign: 'center', margin: 0 }}>
        Zaten hesabınız var mı?{' '}
        <Link href="/login" style={{ color: '#C9A96E', textDecoration: 'none', fontWeight: 600 }}>Giriş yapın</Link>
      </p>
    </div>
  )
}
...
function PricingSection({ onUpgrade }: { onUpgrade: () => void }) {
  ...
  const proPeriod  = yearly ? '/ay (yıllık)' : '/ay'
  const proBadge   = yearly ? '%20 indirim' : null

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      {/* Billing toggle */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem' }}>
        <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', color: !yearly ? '#0C0C0E' : '#A1A1AA', fontWeight: !yearly ? 600 : 400 }}>Aylık</span>
        <button
        ...
        <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', color: yearly ? '#0C0C0E' : '#A1A1AA', fontWeight: yearly ? 600 : 400, display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
          Yıllık {yearly && <span style={{ fontSize: '0.58rem', background: 'rgba(201,169,110,0.15)', color: '#C9A96E', border: '1px solid rgba(201,169,110,0.3)', borderRadius: '4px', padding: '1px 5px', fontWeight: 700 }}>%20 indirim</span>}
        </span>
      </div>
      ...
      <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', color: '#A1A1AA', textAlign: 'center', margin: 0 }}>
        ...
        <Link href="/chat" style={{ color: '#C9A96E', textDecoration: 'none', fontWeight: 600 }}>Ücretsiz başla →</Link>
      </p>
    </div>
  )
}
    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8' }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 30, background: 'rgba(250,250,248,0.95)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(0,0,0,0.07)', padding: '0.75rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <Link href="/login" style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', color: '#71717A', textDecoration: 'none', letterSpacing: '0.05em' }}>Giriş yap</Link>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '3rem 1.5rem 2rem' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <h1 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontStyle: 'italic', fontSize: 'clamp(1.6rem, 3vw, 2.4rem)', fontWeight: 600, color: '#FFFFFF', margin: '0.5rem 0 0.5rem', lineHeight: 1.2 }}>
              Gerçek AI analizini 90 saniyede görün
            </h1>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)', fontWeight: 300 }}>
              Canlı web verisi · Sektör benchmark&apos;ları · Gerçek AI yanıtları
            </p>
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', alignItems: 'center' }}>
            <div>
              ...
              <h2 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: '1.5rem', fontWeight: 600, color: '#0C0C0E', lineHeight: 1.3, margin: '0 0 1rem' }}>
                Gerçek iş sorusuna gerçek AI yanıtı
              </h2>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', color: '#71717A', lineHeight: 1.7, fontWeight: 300, margin: '0 0 1.5rem' }}>
                Eğitim verisi değil — Tavily + Serper&apos;dan çekilen canlı web verisi. Groq 70B bu veriyi sektör benchmark&apos;larıyla sentezler ve spesifik, ölçülebilir öneriler üretir.
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {[
                  '→ 20+ kaynak taranır',
                  '→ Sektör medyanıyla karşılaştırılır',
                  '→ 3 eylem + 30 günlük hedef üretilir',
                ].map(item => (
        ...
        <div style={{ maxWidth: 700, margin: '0 auto', padding: '4rem 1.5rem' }}>
          ...
          <div style={{ background: '#FAFAF8', border: '1px solid rgba(0,0,0,0.09)', borderRadius: '14px', padding: '2rem 2.25rem', marginBottom: '2rem' }}>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', color: '#71717A', margin: '0 0 1.5rem', fontWeight: 300 }}>
              Günlük 5 ücretsiz analiz hakkıyla başlayın. Kredi kartı gerekmez.
            </p>
          ...
          <div style={{ background: '#FAFAF8', border: '1px solid rgba(0,0,0,0.09)', borderRadius: '14px', padding: '2rem 2.25rem', marginBottom: '2rem' }}>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', color: '#71717A', margin: '0 0 1.5rem', fontWeight: 300 }}>
              Ücretsiz ile başlayın, hazır olduğunuzda yükseltin.
            </p>
      ...
      <footer style={{ background: '#0C0C0E', padding: '1.5rem 1.5rem', textAlign: 'center' }}>
        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', color: 'rgba(255,255,255,0.2)', margin: 0 }}>
          © {new Date().getFullYear()} SAIL AI · <Link href="/pricing" style={{ color: 'rgba(255,255,255,0.3)', textDecoration: 'none' }}>Fiyatlar</Link> · <Link href="/chat" style={{ color: 'rgba(255,255,255,0.3)', textDecoration: 'none' }}>Başla</Link>
        </p>
      </footer>
    </div>
  )
